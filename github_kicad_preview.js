    ...
    fetch(rawUrl, {
        ...
        function(response) {
            ...
            response.text().then(function(code) {
                ...
                const fp_pad_circle_re = /\(pad.*\s+(?:np_thru_hole|thru_hole|smd)\s+(?:circle|oval)\s+\(at\s+(?<pos_x>-?\d+(?:\.\d+)?)\s+(?<pos_y>-?\d+(?:\.\d+)?)(?:\s+(?<pos_unk>-?\d+(?:\.\d+)?))?\)\s+\(size\s+(?<size_x>-?\d+(?:\.\d+)?)\s+(?<size_y>-?\d+(?:\.\d+)?)\)(?:\s+\(drill\s+(?<drill>-?\d+(?:\.\d+)?)\))?/g;
